    ...
    if not os.path.exists("skills"):
        ...
        try:
            ...
            if hasattr(resources, 'files'):
                package_skills = resources.files('legal_draft_skill').joinpath('skills')
                ...
                with resources.path('legal_draft_skill', 'skills') as package_skills:
...
def start():
    parser = argparse.ArgumentParser(description="Legal Draft Skill CLI")
    ...
    if args.command == "start":
        ...
        print(f"Starting Legal Draft Skill API on {args.host}:{args.port}")
        print(f"Using Provider: {os.getenv('PROVIDER', 'openrouter')}")
        print(f"Using LLM: {os.getenv('LLM')}")
        uvicorn.run("legal_draft_skill.main:app", host=args.host, port=args.port, reload=False)
    else:
        parser.print_help()
